<template>
  ...
  <NavLink>
    ...
    <div class="col-12 grid-margin">
      <div class="card">
        <div class="card-body">
          <form @submit.prevent="submit(serviceQuotation.id)">
  ...
  Go Back
            ...
            <!-- <div>
    <ul v-if="filteredOptionProIds.length > 0">
      <li v-for="(line, index) in filteredOptionProIds" :key="index">
        <div>{{ line.option_product.name }}</div>
        <div>
          <img v-if="line.option_product.image_url" :src="line.option_product.image_url" alt="Product Image" style="max-width: 100px;" />
        </div>
      </li>
    </ul>
    <p v-else>
      No valid option products found.
    </p>
            </div>-->
            <div class="container">
              <div class="row">
                                ...
                                <div class="col-md-1">
                                  <div class="form-group">
                                    <button
                                      type="button"
                                      @click="removeLine(index,line.id)"
                                      class="btn btn-danger btn-sm mt-4 ml-4"
                                    >
                                      <i class="bi bi-trash"></i>
                                    </button>
                                  </div>
...
const props = defineProps({
  ...
  serviceQuotationOptionLine: {
    type: Array,
    required: true
  },
  serviceQuotationSection: Array,
  serviceQuotationNote: Array,
  errors: Object
});

const taxOptions = ref(props.taxes.map(tax => ({ name: tax.name })));
console.log("ddddddd", props.taxes);
...
const part1 = props.serviceQuotationLine
...
const part2 = props.serviceQuotationSection.map(line => ({
  type: "section",
  id: line.id,
  section: line.section
}));

const part3 = props.serviceQuotationNote.map(line => ({
  type: "note",
  id: line.id,
  note: line.note
}));

const part4 = props.serviceQuotationOptionLine.map(line => ({
  type: "optional",
  id: line.id,
  option_pro_id: line.option_pro_id,
  option_description: line.option_description,
  option_quantity: line.option_quantity,
  option_unit_price: line.option_unit_price
}));

const form = useForm({
  service_id: props.service_id,
  cus_id: props.serviceQuotation.cus_id,
  gst: props.serviceQuotation.gst,
  expiration: props.serviceQuotation.expiration,
  pricelist: props.serviceQuotation.pricelist,
  payment_terms: props.serviceQuotation.payment_terms,
  total: props.serviceQuotation.total,
  tax_amt: props.serviceQuotation.tax_amt,
  untax_amt: props.serviceQuotation.untax_amt,
  sales_person: props.serviceQuotation.sales_person,
  sales_team: props.serviceQuotation.sales_team,
  online_signature: !!props.serviceQuotation.online_signature,
  online_payment: !!props.serviceQuotation.online_payment,
  customer_reference: props.serviceQuotation.customer_reference,
  tags: props.serviceQuotation.tags,
  fiscal_position: props.serviceQuotation.fiscal_position,
  incoterm: props.serviceQuotation.incoterm,
  incoterm_location: props.serviceQuotation.incoterm_location,
  shipping_policy: props.serviceQuotation.shipping_policy,
  delivery_date: props.serviceQuotation.delivery_date,
  source_document: props.serviceQuotation.source_document,
  campaign: props.serviceQuotation.campaign,
  medium: props.serviceQuotation.medium,
  source: props.serviceQuotation.source,
  lines: [...part1, ...part2, ...part3, ...part4].sort((a, b) => a.id - b.id)
});
...
async function removeLine(index, lineId) {
  if (lineId === undefined) {
    ...
    try {
      ...
      await axios.delete(`/quotation/${lineId}`);
      form.lines.splice(index, 1);
    } catch (error) {
