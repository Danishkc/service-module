<template>
  ...
  <NavLink>
    ...
    <div class="col-12 grid-margin">
      <div class="card">
        <div class="card-body">
          <form @submit.prevent="submit(service.id)">
            <div class="d-flex justify-content-end mb-3">
              <ul class="nav nav-tabs">
                ...
                <li class="nav-item">
                  <span class="nav-link active">Confirmed</span>
                </li>
                <li class="nav-item">
            ...
            <div class="ml-3">
              <button class="btn btn-dark btn-md mb-5" @click.prevent="goToNewService">Go Back</button>
              <button class="btn btn-dark btn-md mb-5 ml-3" type="submit">Start Repair</button>
              <button
                type="button"
              ...
              <button
                v-if="props.existingQuotationId!=null"
                @click.prevent="quotationIndex"
                class="btn btn-dark btn-md mb-5 ml-3"
              >View Quotation</button>
              <button
                v-if="props.existingInvoiceId!=null"
                @click.prevent="invoiceIndex"
                class="btn btn-dark btn-md mb-5 ml-3"
              >View Invoice</button>
              <button
                @click.prevent="cancelRepair(service.id)"
                class="btn btn-dark btn-md mb-5 ml-3"
            ...
            <div class="container">
              <div class="row mb-5">
                <h1>
                  <b class="text-4xl text-blue-500">{{props.service.repair_reference}}</b>
                </h1>
              </div>
              <div class="row">
                <div class="col-md-12">
                  <div class="row">
                    ...
                    <div class="col-md-6">
                      ...
                      <div class="form-group d-flex align-items-center">
                        <label style="width: 130px;">Component Status</label>
                        <span
                          :class="service.component_status ? 'text-success mb-2' : 'text-danger'"
                        >{{ service.component_status ? 'Available' : 'Inactive' }}</span>
                      </div>
                    </div>
                  </div>
                        ...
                        <tr v-for="(line, index) in form.lines" :key="index">
                          ...
                          <td>
                            <div class="form-group">
                              <br />
                              <span
                                :class="service.component_status ? 'text-success mt-2' : 'text-danger'"
                              >{{ service.component_status ? 'Available' : 'Inactive' }}</span>
                            </div>
                          </td>
                          <td>
                          ...
                          <td>
                            <div class="form-group">
                              <button
                                @click.prevent="removeLine(index, line.id)"
...
const form = useForm({
  ...
  product_quantity: props.service.product_quantity,
...
async function removeLine(index, lineId) {
  if (lineId === undefined) {
    ...
    try {
      ...
      await axios.delete(`/services/${lineId}`);
      form.lines.splice(index, 1);
    } catch (error) {
...
function cancelRepair(id) {
  router.get("/services/cancelled-confirm-repair/"+id);
}
...
function goToNewService() {
  forms.get(route("services.create"));
}
...
function submit(id) {
  formatDate();
  router.put("/services/end-repair/" + id, form, {
    onSuccess: () => {
